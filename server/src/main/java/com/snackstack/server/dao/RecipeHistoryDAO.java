...
public interface RecipeHistoryDAO {
  ...
  int insertHistory(@Bind("userId") int userId, @Bind("recipeId") int recipeId,
  ...
  @SqlQuery("""
      ...
      """)
  Optional<RecipeHistoryDTO> getHistoryById(@Bind("historyId") int historyId,
      @Bind("userId") int userId);

  /**
   * Mark / un‑mark every history row for this user & recipe as favourite.
   */
  @SqlUpdate("""
          ...
          WHERE recipe_id = :recipeId
            AND user_id   = :userId
      """)
  int updateFavoriteStatusByRecipe(
      @Bind("recipeId") int recipeId,
      @Bind("userId") int userId,
      @Bind("favorite") boolean favorite
  );

  @SqlQuery("""
          SELECT h.history_id as id, h.user_id, h.recipe_id, r.uuid as uuid,
                 r.recipe_name as recipeName, r.description as recipeDescription, 
                 h.created_at as createdAt, h.is_favorite as isFavorite
          FROM recipe_history h
          JOIN recipes r ON h.recipe_id = r.recipe_id
          WHERE h.user_id = :userId
          AND (:favoriteOnly = false OR h.is_favorite = true)
